...
The correlation ID is a simple diagnostic tool. Basically, it's a unique identifier for a business transaction (WHAT IS A BUSINESS TRANSACTION). This identifier is shared across several systems so that each system can print logs with this correlation ID.
